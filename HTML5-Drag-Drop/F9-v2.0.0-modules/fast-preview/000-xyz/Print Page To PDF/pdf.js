...
function PDFViewer(browserApi) {
     ...
     * @type {!PDFMetrics}
     */
    this.metrics = (chrome.metricsPrivate ? new PDFMetricsImpl() : new PDFMetricsDummy());
    this.metrics.onDocumentOpened();
    /**
     * @private {!PDFCoordsTransformer}
     */
    this.coordsTransformer_ = new PDFCoordsTransformer(this.postMessage_.bind(this));
    // Parse open pdf parameters.
    this.paramsParser_ = new OpenPDFParamsParser(this.postMessage_.bind(this));
    var toolbarEnabled = this.paramsParser_.getUiUrlParams(this.originalUrl_).toolbar && !this.isPrintPreview_;
    ...
    this.passwordScreen_.addEventListener('password-submitted', this.onPasswordSubmitted_.bind(this));
