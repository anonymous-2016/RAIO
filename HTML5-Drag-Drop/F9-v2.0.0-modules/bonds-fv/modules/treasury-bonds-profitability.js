...
/**
 * @name treasury-bonds-profitability 国债收益率(中债)
 * creadted 2018.01.23
 * @param {* String} url
...
// treasuryBondsProfitability
OTC_F9_FV.Modules.treasuryBondsProfitability = OTC_F9_FV.Modules.treasuryBondsProfitability || (
    (url = ``, tbody_uid = ``, hc_uid = ``, debug = false) => {
        ...
        .then(
            (json) => {
                ...
                try {
                    ...
                    if (Object.keys(json).length > 0) {
                        ...
                        if (Array.isArray(json)) {
                            ...
                            if (Object.keys(json[0]).length > 0 && typeof(json[0]) === "object") {
                                let tds = document.querySelectorAll(`[data-value="data-otc-TBP"]`),
                                    // tds1, tds2, tds3
                                    date = document.querySelector(`[data-time="data-otc-TBP"]`),
                                ...
                                OTC_F9_FV.Modules.treasuryBondsProfitability.drawHS(datas, hc_uid, false);
                            } else {
                                // no data
                                let uid = `[data-none-uid="otc-treasury-bonds-profitability"]`;
                                const none_div = document.querySelector(uid);
                                // const none_div = document.querySelector(`[data-none-uid="otc-treasury-bonds-profitability"]`);
                                none_div.dataset.none = "no-data-div-visible";
                                // table
                                // const table = document.querySelector(`.otc-treasury-bonds-profitability-table`);
                                // table.dataset.none = "no-data-div-hidden";
                                // const tbody = document.querySelector(`.otc-treasury-bonds-profitability-table-tbody`);
                                const tbody = document.querySelector(tbody_uid);
                                tbody.dataset.none = "no-data-div-hidden";
                            }
                        } else {
                            let message = `handle json error!`,
                                fileName = `treasury-bonds-profitability.js`,
                                lineNumber = 29;
                    ...
                    let url = `file:///E:/**/bonds-fv/modules/treasury-bonds-profitability.js`;
        ...
        /*
        setTimeout((debug = false) => {
            let turn_to_uids = document.querySelectorAll(`[data-turn-to-uid="node-uid-treasury-bonds-profitability-data"]`);
            if (debug) {
...
OTC_F9_FV.Modules.treasuryBondsProfitability.drawHS = OTC_F9_FV.Modules.treasuryBondsProfitability.drawHS || (
    (
        datas = {},
        container_uid = ``,
        debug = false
...
OTC_F9_FV.Modules.treasuryBondsProfitability.init = OTC_F9_FV.Modules.treasuryBondsProfitability.init || (
    (
        {
            ip,
            path,
