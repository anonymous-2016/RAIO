...
/**
 * @name local-governments-debt-aaa-profitability 地方政府债AAA收益率(中债)
 * @author xgqfrms
 * creadted 2018.01.23
 * @param {* String} url
...
// localGovernmentsDebt3AP
OTC_F9_FV.Modules.localGovernmentsDebt3AP = OTC_F9_FV.Modules.localGovernmentsDebt3AP || (
    (url = ``, tbody_uid = ``, hc_uid = ``, debug = false) => {
        ...
        .then(
            (json) => {
                ...
                try {
                    ...
                    if (Object.keys(json).length > 0) {
                        ...
                        if (Array.isArray(json)) {
                            ...
                            if (Object.keys(json[0]).length > 0 && typeof(json[0]) === "object") {
                                let tds = document.querySelectorAll(`[data-value="data-otc-LGD3AP"]`),
                                    // tds1, tds2, tds3
                                    date = document.querySelector(`[data-time="data-otc-LGD3AP"]`),
                                ...
                                setTimeout((debug = false) => {
                                    let export_excel_a = document.querySelector(`[data-excel="otc-local-governments-debt-aaa-profitability-excel"]>a`);
                                ...
                                OTC_F9_FV.Modules.localGovernmentsDebt3AP.drawHS(datas, hc_uid, false);
                            }else{
                                // no data
                                let uid = `[data-none-uid="otc-local-governments-debt-aaa-profitability"]`;
                                const none_div = document.querySelector(uid);
                                // const none_div = document.querySelector(`[data-none-uid="otc-local-governments-debt-aaa-profitability"]`);
                                none_div.dataset.none = "no-data-div-visible";
                                // table
                                // const table = document.querySelector(`.otc-local-governments-debt-aaa-profitability-table`);
                                // table.dataset.none = "no-data-div-hidden";
                                // const tbody = document.querySelector(`.otc-local-governments-debt-aaa-profitability-table-tbody`);
                                const tbody = document.querySelector(tbody_uid);
                                tbody.dataset.none = "no-data-div-hidden";
                            }
                        }else{
                            let message = `handle json error!`,
                                fileName = `local-governments-debt-aaa-profitability.js`,
                                lineNumber = 29;
                    ...
                    let url =`file:///E:/**/bonds-fv/modules/local-governments-debt-aaa-profitability.js`;
        ...
        /*
        setTimeout((debug = false) => {
            let turn_to_uids = document.querySelectorAll(`[data-turn-to-uid="node-uid-local-governments-debt-aaa-profitability-data"]`);
            if (debug) {
...
OTC_F9_FV.Modules.localGovernmentsDebt3AP.drawHS = OTC_F9_FV.Modules.localGovernmentsDebt3AP.drawHS || (
    (
        datas = {},
        container_uid = ``,
        debug = false
...
OTC_F9_FV.Modules.localGovernmentsDebt3AP.init = OTC_F9_FV.Modules.localGovernmentsDebt3AP.init || (
    (
        {
            ip,
            path,
        ...
        // let url = `${ip}${path}?{"ModelId": "${uid}","Compare": "${compare}","CompareDate": "${date}"}`,
        const uid = `bondratefast08`;
        let url = `${ip}${path}?{"ModelId":"${uid}","Compare":"${compare}"${(compare === "2") ? `,"CompareDate":"${date}"` : ``}}`,
            tbody_uid = `[data-tbody="otc-local-governments-debt-aaa-profitability-table-tbody"]`,
            hc_uid = `local-governments-debt-aaa-profitability-hs-container`;
        OTC_F9_FV.Modules.localGovernmentsDebt3AP(url, tbody_uid, hc_uid, false);
    }
);
...
if (OTC_INIT === true) {
    // self init
    OTC_F9_FV.Modules.localGovernmentsDebt3AP.init();
}else{
    // relaod module
}

const localGovernmentsDebt3AP = OTC_F9_FV.Modules.localGovernmentsDebt3AP;
export default localGovernmentsDebt3AP;
export {localGovernmentsDebt3AP};
...
// OTC_F9_FV.Modules.localGovernmentsDebt3AP.init();
// const url = `http://10.1.5.202/webservice/fastview/bond/rate?{"ModelId":"bondratefast08","Compare":"","CompareDate":""}`;
