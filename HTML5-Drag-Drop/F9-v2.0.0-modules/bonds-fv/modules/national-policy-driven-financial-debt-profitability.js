...
/**
 * @name national-policy-driven-financial-debt-profitability 国开政策性金融债收益率(中债)
 * creadted 2018.01.23
 * @param {* String} url
...
// nationalPolicyDrivenFDP
OTC_F9_FV.Modules.nationalPolicyDrivenFDP = OTC_F9_FV.Modules.nationalPolicyDrivenFDP || (
    (url = ``, tbody_uid = ``, hc_uid = ``, debug = false) => {
                    ...
                    if (Object.keys(json).length > 0) {
                        ...
                        if (Array.isArray(json)) {
                            ...
                            if (Object.keys(json[0]).length > 0 && typeof(json[0]) === "object") {
                                let tds = document.querySelectorAll(`[data-value="data-otc-NPDFDP"]`),
                                    // tds1, tds2, tds3
                                    date = document.querySelector(`[data-time="data-otc-NPDFDP"]`),
                                ...
                                OTC_F9_FV.Modules.nationalPolicyDrivenFDP.drawHS(datas, hc_uid, false);
                            }else{
                                // no data
                                let uid = `[data-none-uid="otc-national-policy-driven-financial-debt-profitability"]`;
                                const none_div = document.querySelector(uid);
                                // const none_div = document.querySelector(`[data-none-uid="otc-national-policy-driven-financial-debt-profitability"]`);
                                none_div.dataset.none = "no-data-div-visible";
                                // table
                                // const table = document.querySelector(`.otc-national-policy-driven-financial-debt-profitability-table`);
                                // table.dataset.none = "no-data-div-hidden";
                                // const tbody = document.querySelector(`.otc-national-policy-driven-financial-debt-profitability-table-tbody`);
                                const tbody = document.querySelector(tbody_uid);
                                tbody.dataset.none = "no-data-div-hidden";
                            }
                        }else{
                            let message = `handle json error!`,
                                fileName = `national-policy-driven-financial-debt-profitability.js`,
                                lineNumber = 29;
        ...
        /*
        setTimeout((debug = false) => {
            let turn_to_uids = document.querySelectorAll(`[data-turn-to-uid="node-uid-national-policy-driven-financial-debt-profitability-data"]`);
            if (debug) {
...
OTC_F9_FV.Modules.nationalPolicyDrivenFDP.drawHS = OTC_F9_FV.Modules.nationalPolicyDrivenFDP.drawHS || (
    (
        datas = {},
        container_uid = ``,
        debug = false
...
OTC_F9_FV.Modules.nationalPolicyDrivenFDP.init = OTC_F9_FV.Modules.nationalPolicyDrivenFDP.init || (
    (
        {
            ip,
            path,
        ...
        // let url = `${ip}${path}?{"ModelId": "${uid}","Compare": "${compare}","CompareDate": "${date}"}`,
        const uid = `bondratefast05`;
        let url = `${ip}${path}?{"ModelId":"${uid}","Compare":"${compare}"${(compare === "2") ? `,"CompareDate":"${date}"` : ``}}`,
            tbody_uid = `[data-tbody="otc-national-policy-driven-financial-debt-profitability-table-tbody"]`,
            hc_uid = `national-policy-driven-financial-debt-profitability-hs-container`;
        OTC_F9_FV.Modules.nationalPolicyDrivenFDP(url, tbody_uid, hc_uid, false);
    }
);
...
if (OTC_INIT === true) {
    // self init
    // OTC_COMPARE = "0";
    OTC_F9_FV.Modules.nationalPolicyDrivenFDP.init();
}else{
    // relaod module
}

const nationalPolicyDrivenFDP = OTC_F9_FV.Modules.nationalPolicyDrivenFDP;
export default nationalPolicyDrivenFDP;
export {nationalPolicyDrivenFDP};
