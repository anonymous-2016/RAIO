...
BONDS_BIR_FV.Modules.mainManagementBusiness = BONDS_BIR_FV.Modules.mainManagementBusiness || (
                ...
                try {
                    ...
                    if (emptyChecker(json)) {
                        ...
                        if (Object.keys(datas).length > 0) {
                            ...
                            if (json.datas !== undefined && Array.isArray(json.datas)) {
                                if (json.datas.length > 0) {
                                    ...
                                    // BONDS_BIR_FV.Modules.mainManagementBusiness.drawHS(arr_obj, hsc_uid);
                                    BONDS_BIR_FV.Modules.mainManagementBusiness.drawHS(arr_obj, hsc_uids[0]);
                                    BONDS_BIR_FV.Modules.mainManagementBusiness.drawHS2(arr_obj2, hsc_uids[1]);
                                    // table
                                    let trs = ``;
                                    for (let i = 0; i < arr.length; i++) {
                                        trs += `
                                            <tr class="otc-repurchase-rate-table-tr">
                                                <td class="otc-repurchase-rate-table-td-value" data-value="data-otc-MMB">
                                                    ${arr[i].xm}
                                                </td>
                                                <td class="otc-repurchase-rate-table-td-value" data-value="data-otc-MMB">
                                                    ${arr[i].yysr}
                            ...
                            // no data
                            let thead = document.querySelector(`.otc-repurchase-rate-table-thead`);
                        ...
                        let message = `handle json error!`,
                            fileName = `repurchase-rate.js`,
                            lineNumber = 29;
                        // throw new UserException(message, fileName, lineNumber);
                    }
                } catch (err) {
                    let url =`file:///E:/github/RAIO/HTML5-Drag-Drop/F9-v2.0.0-modules/otc-f9/modules/repurchase-rate.js`;
...
BONDS_BIR_FV.Modules.mainManagementBusiness.drawHS = BONDS_BIR_FV.Modules.mainManagementBusiness.drawHS || (
    (
        datas = {},
        container_uid = ``,
        debug = false
...
BONDS_BIR_FV.Modules.mainManagementBusiness.drawHS2 = BONDS_BIR_FV.Modules.mainManagementBusiness.drawHS2 || (
    (
        datas = {},
        container_uid = ``,
        debug = false
...
BONDS_BIR_FV.Modules.mainManagementBusiness.init = BONDS_BIR_FV.Modules.mainManagementBusiness.init || (
    (
        {
            ip,
            path,
        ...
        let url = `${ip}${path}${socket}${gilcode}`,
            time_uid = `[data-time="otc-repurchase-rate-time"]`,
            ...
            tbody_uid = `[data-tbody="otc-repurchase-rate-table-tbody"]`;
        // copy(Object.keys(json));
        BONDS_BIR_FV.Modules.mainManagementBusiness(url, time_uid, hst_uid, hsc_uids, tbody_uid, false);
        // BONDS_BIR_FV.Modules.mainManagementBusiness(url, time_uid, hst_uid, hsc_uid, tbody_uid, false);
    }
);
...
BONDS_BIR_FV.Modules.mainManagementBusiness.init({
    ip: OTC_IP,
    path: OTC_PATH,
    socket: `/otcperfast09/`,
    gilcode: OTC_GILCODE
